...
HandType calc_hand_type(HashMap* hmap) {
	switch (hmap->r_size) {
		case 1:
			return Five;
		case 2:
...
int main(int args, char* argc[]) {
	...
	map_insert(&card_strengths, 'J', 9);
	map_insert(&card_strengths, 'T', 8);
	map_insert(&card_strengths, '9', 7);
	map_insert(&card_strengths, '8', 6);
	map_insert(&card_strengths, '7', 5);
	map_insert(&card_strengths, '6', 4);
	map_insert(&card_strengths, '5', 3);
	map_insert(&card_strengths, '4', 2);
	map_insert(&card_strengths, '3', 1);
	map_insert(&card_strengths, '2', 0);
	...
	while (fgets(line, 50, file)) {
		...
		for (int i = 0; i < strlen(line); i++) {
			 ...
			 if (i == strlen(line) - 1) {
				 hand.type = calc_hand_type(&repeated_cards);
