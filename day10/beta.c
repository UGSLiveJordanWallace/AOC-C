...
void find(char** map, int m_size, int s_row, int s_col, Directions* ds, Path* p) {
	Vector v;
	v.x = 0;
	v.y = 0;
	int r_origin = s_row;
	...
	for (int i = 0; i < 4; i++) {
		...
		if (v.y != 0 || v.x != 0) {
			...
			p->size++;
			p->p = (Vector*)realloc(p->p, p->size * sizeof(Vector)); 
			p->p[p->size - 1].x = r_origin;
			p->p[p->size - 1].y = c_origin;
			break;
		}
	}
...
int main(int args, char* argc[]) {
	...
	Path p;
	p.size = 0;
	p.p = (Vector*)calloc(p.size, sizeof(Vector));

	find(map, m_size, row, col, &ds, &p);

	int sum = 0;
	for (int i = 0; i < p.size; i++) {
		Vector p1 = p.p[i];
		Vector p2 = p.p[(i+1)%p.size];
		sum += (p1.x * p2.y) - (p2.x * p1.y);
//		printf("(%d, %d)\n(%d, %d) -> (%d, %d)\n------------\n", p1.x, p1.y, p2.x, p2.y, (p1.x * p2.y), (p2.x * p1.y));
	}
	total = (_abs64(sum) - p.size) / 2;
	printf("\nTotal: %llu Size: %u\n", total + 1, p.size);
